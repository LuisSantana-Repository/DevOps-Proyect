...
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/Course" > /dev/null
echo "   ✓ Viewed course list"

echo "3. Alice checks her schedule..."
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/Schedule" > /dev/null
echo "   ✓ Checked schedule"

echo "4. Alice views class information..."
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/Class" > /dev/null
echo "   ✓ Viewed classes"

echo "5. Alice logs out..."
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/login/logout" > /dev/null
echo "   ✓ Logged out"
echo ""
...
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/User" > /dev/null
echo "   ✓ Viewed user list"

echo "3. Bob checks classroom availability..."
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/Classroom" > /dev/null
...
curl -s "$SENTRY_DSN/api/User" > /dev/null
echo "   ✓ Unauthorized access error logged"

echo "3. Accessing non-existent resource..."
curl -s -b $COOKIE_FILE "$SENTRY_DSN/api/User/nonexistent@test.com" > /dev/null
echo "   ✓ Not found error logged"

echo "4. Invalid endpoint access..."
curl -s "$SENTRY_DSN/api/invalidendpoint" > /dev/null
